...
class ModelService {
    ...
    async #updateNodes() {
        ...
        console.log(dtModelNodes, graphServerNodes);
...
async function getBridgeModels() {
    ...
    const bridgeModeModels =
        await bridgeModeModelsModules.getBridgeModeModels();
    ...
    combinedBridgeModels = {
        models: [...models, ...(includeUncurated ? uncurated : [])].filter(
            filterFn
        ),
...
function testHack(models: ModelInfo) {
    ...
    try {
        if (
            new URL(document.location.href).searchParams.has("dtgrpctesthack")
        ) {
